...
function generate_JKS_chain() {
    ...
    keytool -genkey \
        ...
        -keysize   2048 \
        -validity  712 \
        -dname "CN=$NODE_NAME, OU=OpenShift, O=RedHat" \
...
if [[ ! -f $dir/elasticsearch-cluster.jks || -z "$(keytool -list -keystore $dir/elasticsearch-cluster.jks -storepass kspass | grep sig-ca)" ]]; then
  generate_JKS_chain true ${CLUSTER_NAME}-cluster
fi

if [[ ! -f $dir/elasticsearch.jks || -z "$(keytool -list -keystore $dir/elasticsearch.jks -storepass kspass | grep sig-ca)" ]]; then
  generate_JKS_chain false ${CLUSTER_NAME} "$(join , ${CLUSTER_NAME}{,-cluster}{,.${PROJECT}.svc.cluster.local})"
fi
